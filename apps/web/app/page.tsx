...
export default function Home(){
 ...
 return <div className="app-shell">
  {user&&stage==='input'&&<StudioNav current="score"/>}
  <ThemeToggle/>
  {user&&user.id!=='local'&&stage==='input'&&<button className="account-button" onClick={signOut} aria-label={`Sign out ${user.name}`} title={`Signed in as ${user.name} · Sign out`}><LogOut size={17}/></button>}
  <main>
