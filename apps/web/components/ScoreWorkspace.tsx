...
export default function ScoreWorkspace({result,source,onNew}:{result:ScoreResult;source:Source;onNew:()=>void}){
 ...
 return <section className={`workspace view-enter ${editingEnabled?'':'read-only-score'}`}>
  <div className="workspace-heading"><div><div className="eyebrow">YOUR MUSIC, ON PAPER</div><h1>{source.title}</h1><p>{source.artist||'Your recording'} <span>·</span> {edited?'Edited score':'Original AI score'}</p></div><div className="heading-actions"><button className="new-score-button" onClick={()=>{try{sessionStorage.setItem(MELODY_KEY,abc);}catch{}location.assign('/music');}} title="Generate a new song that follows this melody">Make music from this</button><button className="new-score-button" onClick={onNew}><span aria-hidden="true">+</span> New score</button></div></div>
