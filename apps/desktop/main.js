// Kiku Studio desktop: the web studio in a window, with its models running on
...
const studioDir = process.env.SCORE_STUDIO_DIR || path.join(repo, 'apps', 'web');
const engineBinary = process.env.SCORE_ENGINE_BIN ||
  path.join(repo, 'engine', 'bin',
    process.platform === 'win32' ? 'audiocpp_server.exe' : 'audiocpp_server');
const modelsDir = process.env.SCORE_MODELS_DIR || path.join(repo, 'models');
const children = [];
...
function engineConfig(port) {
  ...
  // The engine looks session options up by "<family>.<option>".
  for (const model of models) {
    model.session_options = Object.fromEntries(
      Object.entries(model.session_options).map(([key, value]) => [`${model.family}.${key}`, value]));
  }
  return {
    host: '127.0.0.1', port,
...
app.whenReady().then(start).catch((error) => {
  stop();
  dialog.showErrorBox('Kiku Studio could not start', error.message);
  app.exit(1);
});
